package ru.spbstu.icst.lunarrover.levels;
...
import ru.spbstu.icst.lunarrover.controls.CButton;
import ru.spbstu.icst.lunarrover.controls.JoyStick;
import ru.spbstu.icst.lunarrover.controls.JumpGauge;
import ru.spbstu.icst.lunarrover.player.IBody;
import ru.spbstu.icst.lunarrover.player.Player;
import ru.spbstu.icst.lunarrover.player.UserData;
import ru.spbstu.icst.lunarrover.screens.LevelCompletedScreen;
import ru.spbstu.icst.lunarrover.screens.LevelFailedScreen;
import ru.spbstu.icst.lunarrover.screens.PausedScreen;
...
public class Level extends StageGame{
    ...
    public Level(String directory) {
        ...
        pleaseWait = new Image(LunarRover.atlas.findRegion("please_wait"));
    ...
    private void setBackGround(String region) {
        clearBackground();
        Image bg = new Image(LunarRover.atlas.findRegion(region));
    ...
    private void build() {
        ...
        jumpBackBtn = new CButton(
                new Image(LunarRover.atlas.findRegion("jump1")),
                new Image(LunarRover.atlas.findRegion("jump1_down")),
                mmToPx(10)
        );
        ...
        jumpForwardBtn = new CButton(
                new Image(LunarRover.atlas.findRegion("jump2")),
                new Image(LunarRover.atlas.findRegion("jump2_down")),
                mmToPx(10)
        );
    ...
    public void setMusic(String name) {
        musicName = name;
        LunarRover.media.addMusic(name);
    }
    ...
    public void dispose() {
        if (musicName != null && musicHasLoaded) {
            LunarRover.media.stopMusic(musicName);
            LunarRover.media.removeMusic(musicName);
        }
    ...
    private void playMusic() {
        if (musicName != null && musicHasLoaded) {
            LunarRover.media.playMusic(musicName, true);
        }
    }
        ...
        if (data != null) {
            if (data.name.equals("land") && !player.isHasDestroyed()) {
                if (player.getRotation() < -90 || player.getRotation() > 90) {
                    player.destroy();
                    LunarRover.media.playSound("crash.ogg");
                    levelFailed();
                } else {
                    player.touchGround();
                }
    ...
    private void levelCompleted() {
        ...
        LunarRover.media.playSound("level_completed.ogg");
        LunarRover.media.playMusic("level_win.ogg", false);
    }
