...
public class LevelFailedScreen extends Group {
    ...
    public LevelFailedScreen(float w, float h) {
        ...
        title = new Image(LunarRover.atlas.findRegion("game_over"));
        title.setX((w - title.getWidth()) / 2);
        title.setY(h);
        addActor(title);

        retry = new ImageButton(
                new TextureRegionDrawable(LunarRover.atlas.findRegion("retry_btn")),
                new TextureRegionDrawable(LunarRover.atlas.findRegion("retry_btn_down"))
        );
        addActor(retry);
